...
namespace CustomUI
{
    ...
    public class DropdownEx : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        ...
        private void AlphaFadeList(float duration, float targetAlpha)
        {
            if (m_Dropdown == null)
                return;
            ...
            m_FadeCoroutine = StartCoroutine(FadeCanvasGroup(group, duration, targetAlpha));
        }

        private IEnumerator FadeCanvasGroup(CanvasGroup group, float duration, float target)
        {
            if (group == null)
                yield break;
